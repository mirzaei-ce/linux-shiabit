...
###   This script attempts to download the signature file SHA256SUMS.asc from shiabit.org
...
WORKINGDIR="/tmp/shiabit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://shiabit.org/bin/"
VERSIONPREFIX="shiabit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'shiabit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Shiabit Core binary release signing key installed?"
   fi
