Shiabit Core version 0.11.0 is now available from:

  <https://shiabit.org/bin/shiabit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Shiabit Core or other software:
...
For example, add the following to `shiabit.conf`:
...
related to the blockchain in the shiabit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Shiabit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Shiabit Core will 
...
For an example script see [shiabit-submittx](https://github.com/laanwj/shiabit-submittx).
...
- #5334 `cf87536` libshiabitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new shiabit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ShiabitAmountField
- #5753 `0f44672` Add shiabit logo to about screen
...
- #5809 `46bfbe7` Add shiabit-cli man page
...
- #5849 `41259ca` contrib/init/shiabitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Shiabit-Qt.app to Shiabit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for shiabit-tx)
